...
Research and implementation date: **15 September 2026**. Prepared for Toni, a Fall 2021 entrant planning around Fall 2026 completion. Official author name, conferral quarter, defense date and committee appointments remain unconfirmed.

## Executive decisions

The delivered starter is a self-contained writing foundation with three provisional research chapters, an integrative introduction, a synthesis, and a removable formatting appendix. It contains no invented abstract, results, contributions, acknowledgments or approval. It is not ready for institutional filing.
...
Use the pinned community `ucscthesis` class with small project-level corrections. Use native Overleaf GitHub synchronization, start with a new Overleaf upload, and let Overleaf create the new private GitHub repository. Keep research repositories separate and import only reviewed, attributed assets at recorded commits. The setup steps are in `docs/WORKFLOW.md`.
...
**Recommended provisional choice:** research chapters with a shared introduction and final synthesis. Reserve three research slots; do not equate the slots with three proven independent contributions. The author/advisors must approve chapter counting and any splitting of one project. If the audit supports only two sufficiently distinct contributions, report the gap instead of manufacturing a third.

Proposed contents: (1) Introduction and common framework; (2) Research A: dynamic quantile forecasting; (3) Research B: joint quantile inference; (4) Research C: interval-targeted methods; (5) Synthesis, limitations and future work; appendices; bibliography. The detailed source, prerequisite, new-exposition and overlap analysis is in `docs/chapter-plan.md`.
