...
\begin{singlespace}
\noindent\textit{This chapter integrates two closely related manuscript lines by Antonio de Leon, Raquel Prado, and Bruno Sans\'o. The fixed-content interval theory, tolerance procedure, and validation study provide the main spine. Proposed regression and dynamic endpoint extensions are developed in the same body, with proofs and computational derivations placed beside the claims they support.}
\end{singlespace}
...
procedure. TCSP chooses a retained order-statistic count by numerical uniform-scan
calibration and reports the shortest closed window at that count. The exact
scan recursion and a finite-sample proof matched to this reported closed-window
action remain open; the completed simulation study therefore supplies
repeated-sampling evidence rather than an exact distribution-free theorem for
the adaptive rule. Generalized-posterior endpoint summaries are treated
separately. We also study
...
This target geometry leads to an empirically calibrated tolerance procedure. The shortest
...
calibrate a retained count numerically from the uniform scan statistic, then report the
shortest closed order-statistic window at that count. The validation study
assesses the resulting tolerance statement, while an exact finite-sample proof
for this particular action remains open. Any generalized-posterior calculation after
\((c,\delta)\), or after a selected retained count, describes endpoint
uncertainty for a fixed target; it does not supply the open finite-sample result
for the calibrated empirical action.
...
loss, not a distribution-free tolerance theorem. The final part of this
chapter develops theoretical generalized-Bayes endpoint regression,
basis-expansion endpoint models, and time-varying endpoint models. The audited
extension snapshot contains derivations but no snapshot-matched implementation,
experiments, tests, or provenance manifest; these constructions are consequently
presented as proposed methods rather than validated procedures.
...
The formal setting is intentionally bounded. The tolerance study concerns iid
continuous univariate samples and the closed order-statistic
...
The chapter proceeds as follows. Section~\ref{rqr:sec:interval-functionals} defines
...
pharmaceutical application. The final sections develop proposed regression and
dynamic extensions and synthesize the evidentiary limits.
...
\subsection{Probability-window geometry}
...
\subsection{Score derivation and consequences}
...
\input{tables/ch05-mti/mean_tilt_cf_mini_study.tex}
...
\subsection{Identification proofs and restricted-score equations}
...
\subsubsection{Fixed-content target and proof}
...
\subsection{Training-sample balance}
...
\subsection{Fixed-target generalized-Bayes mode}
...
selection. The full fixed-target equations appear below; broader Gibbs,
regression, basis-expansion, and time-varying endpoint constructions are
developed in Section~\ref{mti:sec:targets}.

\subsection{Direct content-probability check}
...
candidates in a separate calibration run. It is a model-based calibration
device for the comparator, not a frequentist tolerance guarantee.

\subsection{Fractional empirical-balance theorem}
...
The pointwise score geometry is immediate from these equations. For example,
at \(c=0.80\), \(m=0\), and \(h=1\), the residual product is negative exactly
inside the interval. Covered observations contract the half-width score and
misses expand it; a fixed \(\delta\) shifts the midpoint score without changing
the half-width score. This is a deterministic score calculation, not fitted
evidence.

\subsection{Fixed-target MTI-ECM calculation}
...
updates an interval-root target under a loss; empirical validation of its mode
does not convert the generalized posterior into a tolerance guarantee.
...
calibration run. It is a posterior probability under a response-distribution
model, not the loss-based endpoint posterior and not a distribution-free
tolerance statement.
...
Here TCSP denotes the tolerance-calibrated shortest-path procedure:
...
\subsection{Validation design and results}
...
separate from the classical distribution-free order-statistic results and does
not close the action-matched TCSP proof gap.
...
across distributions with different scales and tail behavior. The shortest
population content-\(c\) interval for each known simulation law is used as a
lower width reference rather than as a sample-based tolerance procedure.
...
\CompactTableStyle
\input{tables/ch05-mti/tolerance_validation_article_scenario_ranges.tex}
\end{table}

Distribution-specific empirical 2.5\%--97.5\% width ranges were also compared
with the shortest population content-\(c\) interval under each known simulation
law. These raw-scale comparisons were made within distribution, not pooled
across laws with different tail behavior and scales.
...
Table~\ref{rqr:tab:tolerance-validation-primary} gives the attainment
comparison without pooling widths across distributional scenarios. The
calibrated scan procedure targets the requested content--confidence statement.
In the independent
...
The source reproducibility record contains the distribution-level
content-attainment and width tables behind these summaries. The retained
chapter displays give the selected MTI-ECM calibration summary and the TCSP
retained counts; the feasibility thresholds are stated directly below.
...
\subsubsection{Protocol, calibration, and feasibility details}
...
\input{tables/ch05-mti/tolerance_validation_mti_ecm_selected_policy_summary.tex}
...
\TableStyle
\input{tables/ch05-mti/tolerance_validation_article_scan_calibration.tex}
\end{table}
...
stability check on the withheld batches. The nominal content--confidence pair
is the design target, not a guarantee inferred from these held-out fractions.
...
\TableStyle
\input{tables/ch05-mti/pharma_application_primary_summary.tex}
\end{table}
...
study. The next subsection repeats the analysis for tablet-core weight relative
...
\subsection{Data, secondary response, and sensitivity analyses}
...
\TableStyle
\input{tables/ch05-mti/pharma_application_supplement_summary.tex}
\end{table}
...
\subsection{Synthesis of the iid interval results}
...
model-based content probabilities. The TCSP count is obtained by numerical scan
calibration, and its observed content--confidence behavior is assessed in the
independent validation study. Moving a guarantee from one layer to another
requires a separate theorem or direct validation study.
...
Several limitations remain. The intended TCSP statement is iid, continuous, and
univariate, and it uses the stated closed order-statistic convention. Boundary
tilts can lead to one-sided limits or inactive roots, and tilts
...
problems. The proposed endpoint-regression and dynamic constructions below
therefore carry different evidence standards from the empirically evaluated iid
tolerance procedure.
...
indexed by retained-mean balance. This section derives generalized-Bayes
...
steps. These are theoretical computational constructions: the audited source
snapshot contains no matching implementation, experiments, tests, or provenance
manifest. Tolerance guarantees and response prediction would require separate
calibration or response models.

\subsection{Scope and inferential target}
...
content but differ in endpoint placement. The preceding sections develop this
geometry and use it to define an empirically scan-calibrated tolerance
procedure. The extensions address a different question: once a fixed-content target
...
Here MPI and MTI remain loss-defined targets. For a fixed content
...
The proposed development has three parts. Static endpoint regression for MPI
and fixed-tilt MTI uses root-label symmetry and midpoint-gap summaries for
interpretation. A pseudo-asymmetric-Laplace augmentation yields Gaussian
conditional root updates, generalized-inverse-Gaussian latent-scale updates,
and a deterministic ECM mode calculation. The same fixed-target construction
then extends to deterministic basis expansions and dynamic linear endpoint
states through alternating root-specific smoothing updates.
...
functionals under the chosen loss and prior; they do not inherit tolerance
validity from the iid simulation study.
...
content-confidence tolerance procedure assessed for its reported action.
Forecast evaluation requires a response-distribution model or a
prespecified predictive interval rule.
...
retained-mean balance. The generalized-Bayes construction gives formal
regularized endpoint regression, deterministic basis-expansion endpoint models,
and time-varying endpoint models for a chosen fixed-content target. The
pseudo-AL augmentation yields conditional Gibbs and ECM equations while
preserving the distinction between a loss update and a response likelihood.
Implementation and empirical performance of these extensions are not
established by the audited snapshot.
...
\subsection{Computational derivation details}
...
This chapter separates four statistical objects that are easily conflated.
MPI and MTI are population interval functionals defined by content and
retained-mean balance. A generalized posterior represents parameter or endpoint
uncertainty for one fixed loss-defined target. A response-distribution model can
produce predictive response or interval-content probabilities. A tolerance
procedure instead requires a repeated-sampling content--confidence statement
for the action actually reported. None of these interpretations follows
automatically from another.

The population results identify the admissible mean-tilt path and provide one
authoritative characterization and global-minimization argument under the
stated continuity, support, density, and moment conditions. The empirical
score equations provide exact fractional training-sample balance, not exact
future coverage. The tolerance study evaluates TCSP, MTI-ECM, Young--Mathew,
and Wilks intervals over 72 feasible distribution-level cells. Its findings are
repeated-sampling evidence for the frozen procedures. Exact scan recursion and
a finite-sample proof matched to the adaptive closed-window TCSP action remain
open, so the numerical calibration is not restated as an analytic theorem.

The pharmaceutical analysis has a correspondingly limited role: it illustrates
endpoint placement and held-out stability in 187 historical product-23 batches,
not regulatory validity or performance under dependent production sampling.
The regression, basis-expansion, and dynamic endpoint formulations show how the
same fixed-target loss could be used beyond the intercept-only setting. Because
the audited extension snapshot supplies derivations but no matching software,
tests, experiments, or provenance manifest, those formulations remain proposed
methods. Their implementation, computational validation, response-predictive
interpretation, and tolerance calibration are separate tasks.
