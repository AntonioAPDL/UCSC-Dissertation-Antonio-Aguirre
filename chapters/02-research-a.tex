...
The contribution developed in this chapter is software-centered. The \proglang{R} package \pkg{exdqlm} turns these methodological components into an integrated workflow for specifying, fitting, inspecting, diagnosing, forecasting, and reproducing Bayesian dynamic and static quantile analyses. The package makes five main software contributions:
\begin{itemize}
...
The \code{exdqlm} package is distributed under the MIT License and is available from the Comprehensive R Archive Network at \url{https://CRAN.R-project.org/package=exdqlm}. The analyses in this chapter are fixed to version 1.1.1, a reproducibility patch built on the public 1.1.x package line. Chapter~\ref{ch:research-b} uses the earlier 1.1.0 interface recorded by its application snapshot, whereas version 1.1.2 is the later support snapshot audited for compatibility. These versions are related but not interchangeable provenance records: neither 1.1.0 nor 1.1.2 is substituted for the 1.1.1 analysis reported here.

The chapter first establishes the distributional and state-space framework, then describes the package architecture and its numerical implementation. Selected posterior targets and computational blocks connect that interface to the underlying statistical calculations. Four examples then evaluate the workflow for dynamic fitting, forecasting, predictive synthesis, transfer effects, and sparse static regression. The final section summarizes the supported scope and its limitations.
...
The integrated technical material connects the package interface with the
...
and \proglang{R} backend paths, including ELBO monitoring where applicable, are
...
The package interface follows three main design principles. First, model specification is separated from estimation so that the same state-space model can be used with different inference engines. Second, engine-specific S3 classes are combined with shared family classes to provide common behavior without breaking existing code. Third, post-processing operations return explicit reusable objects, separating computation from printing, summarization, and visualization. Table~\ref{exd:table:flow} summarizes the resulting workflow.
...
Computationally intensive operations have \proglang{C++} implementations through \pkg{Rcpp}, with \proglang{R} implementations retained for numerical parity checks and portability. These include the exported exAL density, distribution, quantile, and random-generation utilities; selected Kalman recursions and model builders; posterior and latent-variable sampling paths; posterior predictive simulation; and MCMC backend routing. Package options control backend routing for Kalman recursions, model builders, posterior sampling, posterior predictive simulation, and MCMC routing. In version 1.1.1, compiled stochastic helper paths used by the recorded analyses run through serial \proglang{R}-controlled random-number streams, rather than worker-thread or private C++ random-number streams.
...
The reported results were generated with \proglang{R} 4.6.0 and \pkg{exdqlm} 1.1.1 on x86\_64 Linux. The external replication script sets \code{RNGversion("4.6.0")} and \code{RNGkind("Mersenne-Twister", "Inversion", "Rejection")} before applying the recorded seeds. Native thread variables for OpenMP and common BLAS implementations are set to one before launching \proglang{R}; the benchmark settings used in the examples enable the \code{"fast"} C++ MCMC backend and keep stochastic compiled helper streams controlled by the \proglang{R} random-number generator. The audited source archive contains \code{code.R}, intended to run with \code{R CMD BATCH --vanilla code.R code.Rout}; it records fitted-object output, the example tables, selected scalar summaries, \code{sessionInfo()}, and the figure assets used here. The source archive remains external to the dissertation build. Runtime entries are elapsed fitting times on the recorded reference platform and are not machine-independent constants.
These computational controls affect implementation and performance rather than the statistical models being fitted.

\begin{table}[!htbp]
\centering
% This API reference is the chapter's widest and longest retained table.
% Compact spacing is used locally so the table remains on one page.
\CompactTableStyle
...
As an illustration we fix a single discount factor of $0.9$ to define the evolution covariance matrix $\mat{W}_t$.
 ...
 In the fits shown here we use \code{n.burn = 2000} and \code{n.mcmc = 3000}. The computation uses the backend settings described in Section \ref{exd:sec:design}; backend and runtime provenance for the recorded run are retained in the external replication logs.
...
\(\gamma\) is \(-0.039\) with a 95\% credible interval
\((-0.397, 0.291)\), while the posterior mean of \(\sigma\) is \(0.375\) with
a 95\% credible interval \((0.302, 0.460)\). Retained samples are \code{mcmc}
...
Several development directions follow from these limitations: multivariate-output dynamic quantile models that represent cross-series dependence through a modeled correlation matrix for related time series; further assessment of variational approximation error for the nonconjugate \((\gamma,\sigma)\) skewness--scale block; nonlinear dynamic quantile modeling beyond the linear state-space form used here; and joint or regularized fitting across quantile levels. The chapter prose, integrated technical material, and external replication sources have separate roles: the chapter describes the statistical workflow and records the posterior targets and package-specific computational blocks needed to interpret it, while the fixed source archive regenerates the figures, tables, selected fitted-object output, and benchmark information.
