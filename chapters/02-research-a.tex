...
The \code{exdqlm} package is distributed under the MIT License and is available from the Comprehensive R Archive Network at \url{https://CRAN.R-project.org/package=exdqlm}. The package source submitted with this article is version 1.1.1, a reproducibility patch built on the public 1.1.x package line.

The remainder of the paper is organized as follows. Section \ref{exd:sec:exdqlm} summarizes the modeling framework, including algorithms used for posterior inference and transfer-function state augmentation for nonlinear input effects. It also discusses static quantile regression as a special case of the framework. Section \ref{exd:sec:diags} details the model diagnostics and forecasting method implemented in the package. Section \ref{exd:sec:design} describes the package design and implementation, including the object families and standard methods used by the examples. Section \ref{exd:sec:exs} illustrates the package through four examples. Section \ref{exd:sec:summary} summarizes the package capabilities, and the integrated technical sections collect selected posterior targets, package-specific computational blocks, diagnostic formulas, and predictive-synthesis details used by the implementation.
...
The integrated technical material aims to connect the package interface discussed in the article with the
...
This section provides an overview of extended dynamic quantile linear models (exDQLMs) along with related posterior inference and tools for model diagnostics. Additional implementation details and posterior derivations are provided in the integrated technical sections below.
...
Table \ref{exd:table:post} shows selected core inference controls for the fitting functions \code{exdqlmMCMC()} and \code{exdqlmLDVB()}.
The MCMC routine also accepts \code{Sig.mh} as an optional proposal covariance when \code{mh.proposal} selects a joint random-walk MH kernel; it is not used under the default scale-collapsed slice update.
Both fitting routines also use package-level options to control selected compiled and R backend paths. These runtime options, including ELBO monitoring where applicable, are described in Section
 \ref{exd:sec:design} and are distinct from the function-specific inference controls summarized in Table \ref{exd:table:post}.
...
The arguments \code{df} and \code{dim.df} specify the component discount factors $(\delta_1,\dots,\delta_h)$ and their corresponding dimensions $(q_1,\dots,q_h)$.  Section \ref{exd:sec:exs} provides examples.  \cite{exd-west1997bayesian} and \cite{exd-prado_ferreira_west2021} give a detailed treatment of component discounting.
...
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.15\textwidth}>{\centering\arraybackslash}p{0.15\textwidth}>{\raggedright\arraybackslash}p{0.14\textwidth}>{\raggedright\arraybackslash}X@{}}
	...
	\bottomrule
\end{tabularx}
\caption[Transfer-function inputs]{Transfer-function inputs for \code{exdqlmTransferLDVB()} and \code{exdqlmTransferMCMC()}. Here $k$ denotes the number of transfer covariates.}
\label{exd:table:tf}
\end{table}
...
$\epsilon_t \sim \exAL_{p_0}(0,\sigma,\gamma=0) = \AL_{p_0}(0,\sigma),$ leading to the dynamic quantile linear model (DQLM) of \cite{exd-gonccalves2017dynamic}. The DQLM can be implemented with our package using the parameter \code{dqlm.ind = TRUE}.
...
Finally, combining these two restrictions gives static Bayesian linear quantile regression under the AL working likelihood \cite{exd-yu2001bayesian}, as implemented in packages such as \pkg{bayesQR}. Within the \pkg{exdqlm} package, this model can be fitted using \code{exalStaticMCMC()} or \code{exalStaticLDVB()} with \code{al.ind = TRUE}, a static convenience alias for \code{dqlm.ind = TRUE}, which fixes $\gamma=0$.
...
where $\mat{a}_{\tilde{t}}(k) = \mat{G}_{\tilde{t}+k} \mat{a}_{\tilde{t}}(k-1)$, $\mat{R}_{\tilde{t}}(k) = \mat{G}_{\tilde{t}+k} \mat{R}_{\tilde{t}}(k-1) \mat{G}_{\tilde{t}+k}^\top + \mat{W}_{\tilde{t}+k}$, $\mat{a}_{\tilde{t}}(0) = \mat{m}_{\tilde{t}}$, and $\mat{R}_{\tilde{t}}(0) = \mat{C}_{\tilde{t}}$, with $\mat{m}_{\tilde{t}}$ and $\mat{C}_{\tilde{t}}$ denoting the filtered mean and covariance of $\vect{\theta}_{\tilde{t}}$, respectively. This distribution is implemented in the standard \code{predict()} method for fitted dynamic objects and is available for both MCMC and LDVB fits. The fitted object supplies the filtered posterior summaries at the forecast origin, while optional future observation and evolution matrices are supplied when the required forecast design is not already stored in the fitted model. Table \ref{exd:table:fore} summarizes the forecast inputs and optional posterior predictive draw controls. The returned
...
recursions for this Gaussian block \citep{exd-west1997bayesian}.
\label{exd:sec:gaussian_state_update}

\subsection{Dynamic exDQLM under the exAL likelihood}
\label{exd:sec:dyn_exdqlm}

The dynamic exDQLM augments the AL hierarchy with $s_t\sim\TNp(0,1)$ and
the exAL constants in \eqref{exd:eq:exal_ABC}:
\begin{align}
...
\label{exd:sec:appendix_transfer_function}

The notation below follows the transfer-function formulation in Section
\ref{exd:sec:tf}. Let \(\mat{x}_t=(x_{1t},\ldots,x_{kt})^\top\) denote the
...
through the transfer-function discount factors. In Example \ref{exd:sec:ex3},
...
\label{exd:eq:static_beta_ridge}
\end{equation}
...
s_i\sim\TNp(0,1).
\label{exd:eq:static_exal_hierarchy}
\end{align}
The posterior target is
\begin{align}
...
\label{exd:eq:static_exal_target}
\end{align}
...
Section~\ref{exd:sec:static_rhs}.

\section{LDVB scale--skewness implementation for exAL models}
\label{exd:sec:ldvb_details}

The nonconjugate part of the exAL posterior is the joint scale--skewness
block $(\sigma,\gamma)$. The LDVB routines in \pkg{exdqlm} specialize
nonconjugate variational inference \citep{exd-wang2013nonconjugatevb} to this
block. In version 1.1.1, the default implementation uses a structured
\(q(\gamma)q(\sigma\mid\gamma)\) factor that preserves the conditional GIG
scale kernel. The earlier two-dimensional Laplace--delta factor on transformed
coordinates remains available for comparison and backward-compatible
workflows.
...
\label{exd:eq:dyn_exdqlm_vb_precision}
\end{equation}
and pseudo-response
\begin{equation}
y_t^{\mathrm{VB}}
...
as in Section~\ref{exd:sec:dyn_dqlm}.
...
The default structured factor works on the bounded logit coordinate
\begin{equation}
z_\gamma=\logit\left(\frac{\gamma-L}{U-L}\right).
\label{exd:eq:dyn_exdqlm_ld_transform}
\end{equation}
For each candidate value of \(\gamma\), the conditional factor
\(q(\sigma\mid\gamma)\) has a GIG kernel. The implementation evaluates the
collapsed objective for \(z_\gamma\), which integrates out \(\sigma\), and then
computes the expectations needed in \eqref{exd:eq:dyn_exdqlm_kappa} using the
quadrature weights for \(q(\gamma)\) and GIG moments for
\(q(\sigma\mid\gamma)\). Approximate posterior draws are obtained by sampling
\(\gamma\) from this discrete representation and then drawing \(\sigma\) from
the corresponding conditional GIG distribution.

The legacy Laplace--delta factor instead uses transformed coordinates
\[
\ell_\sigma=\log\sigma,\qquad z_\gamma=
\logit\left(\frac{\gamma-L}{U-L}\right).
\]
If $r=\expit(z_\gamma)$, its transformed objective is
\begin{align}
...
\label{exd:eq:dyn_exdqlm_ld_objective}
\end{align}
The legacy factor maximizes this objective, uses the local Hessian to form a
...
scale--skewness factor description, derived moments, ELBO trace, and
...
Section~\ref{exd:sec:app_distributional_conventions}. For AL/DQLM fits,
...
\label{exd:sec:static_rhs}

The \code{rhs\_ns} option implements the conditionally conjugate
regularized horseshoe formulation of \citet{exd-nishimura2023shrunken} for
...
\label{exd:eq:rhs_ns_vb_updates}
\end{align}
...
\label{exd:sec:forecast_diagnostics_synthesis}
...
\label{exd:eq:crps_estimator}
\end{equation}
where the weights sum to one \citep{exd-gneiting2007,exd-laio2007verification}. The
...
Computationally intensive operations have \proglang{C++} implementations through \pkg{Rcpp},  with \proglang{R} implementations retained for numerical parity checks and portability. These include the exported exAL density, distribution, quantile, and random-generation utilities; selected Kalman recursions and model builders; posterior and latent-variable sampling paths; posterior predictive simulation; and MCMC backend routing. Package options control backend routing for Kalman recursions, model builders, posterior sampling, posterior predictive simulation, and MCMC routing. In version 1.1.1, compiled stochastic helper paths used by the manuscript run through serial \proglang{R}-controlled random-number streams, rather than worker-thread or private C++ random-number streams.

The variational routines return standardized traces for the ELBO, scale, skewness, and convergence diagnostics, with the closed-form, structured scale--skewness, and legacy Laplace--delta ELBO components documented in the integrated technical sections. The examples use fixed benchmark settings, recorded random-number seeds, and a documented reference environment so that run times and stochastic outputs are reproducible.

\paragraph{Computational details and replication.}
The manuscript results were generated with \proglang{R} 4.6.0 and \pkg{exdqlm} 1.1.1 on x86\_64 Linux. The replication script sets \code{RNGversion("4.6.0")} and \code{RNGkind("Mersenne-Twister", "Inversion", "Rejection")} before applying the recorded seeds. Native thread variables for OpenMP and common BLAS implementations are set to one before launching \proglang{R}; the benchmark settings used in the examples enable the \code{"fast"} C++ MCMC backend and keep stochastic compiled helper streams controlled by the \proglang{R} random-number generator. The submitted replication archive contains a flat \code{code.R} script intended to be run with \code{R CMD BATCH --vanilla code.R code.Rout}. That run prints fitted-object output, Tables 7--10, selected scalar summaries, and \code{sessionInfo()} to \code{code.Rout}, and writes the manuscript figures to \code{Rplots.pdf} and to the PNG files used by this article. Runtime entries in the tables are elapsed fitting times on the recorded reference platform and are not intended to be machine-independent constants.
...
\section{Nonconjugate VB and backend notes}
\label{exd:sec:numerical_blocks}

For the legacy Laplace--delta factor, let $z$ denote the transformed
nonconjugate parameter vector with mode
$\hat z$ and negative Hessian $\mat{H}$ at the mode. The LDVB block uses
\begin{equation}
q(z)=\N(\hat z,\mat{H}^{-1}).
\label{exd:eq:ld_gaussian}
\end{equation}
...
\label{exd:eq:entropy_ld}
\end{equation}
The Jacobian terms for transforming from $(\ell_\sigma,z_\gamma)$ back to
$(\sigma,\gamma)$ are included in the objective in
\eqref{exd:eq:dyn_exdqlm_ld_objective}. For the default structured factor, the
package combines the entropy of the discrete/quadrature approximation to
\(q(\gamma)\) with the conditional GIG entropy for
\(q(\sigma\mid\gamma)\). Other ELBO terms use standard entropy identities for
Gaussian, inverse-gamma, GIG, and positive truncated-normal factors under the
parameterizations in
Section~\ref{exd:sec:app_distributional_conventions}. Those bookkeeping terms are
...
nonconjugate skewness updates. Under the default scale-collapsed kernel, the
slice sampler is applied on the bounded logit scale after analytically
integrating out \(\sigma\), and \(\sigma\) is then redrawn from its exact GIG
conditional distribution. Legacy slice and MH kernels remain available through
\code{mh.proposal}; their target kernels are the conditional log posteriors
induced by the exAL augmentation.

\label{exd:sec:app_backend_options}
...
As an illustration we fix a single discount factor of $0.9$ to define the evolution covariance matrix $\mat{W}_t$.
 ...
 In the fits shown here we use \code{n.burn = 2000} and \code{n.mcmc = 3000}. The computation uses the backend settings described in Section \ref{exd:sec:design}; backend and runtime provenance for the manuscript run are recorded in the replication logs.
...
By default, \code{exdqlmMCMC()} uses the package's LDVB routine, \code{exdqlmLDVB()}, to warm-start the chain. This initialization choice is separate from the subsequent MCMC kernel. Because the data are relatively symmetric, we do not expect the skewness parameter \(\gamma\) to be far from zero at \(p_0 = 0.5\). For the diagnostic display, we run the unrestricted median model separately as \code{M50.trace} with a longer burn-in. Consistent with the symmetry of the data, the posterior mean of \(\gamma\) is \(-0.039\) with a 95\% credible interval \((-0.397, 0.291)\), while the posterior mean of \(\sigma\) is \(0.375\) with a 95\% credible interval \((0.302, 0.460)\). All retained samples in the output of \code{exdqlmMCMC()} are \code{mcmc} objects, so we can use the package \pkg{coda} \citep{exd-coda} to examine the trace and density plots of both \(\sigma\) and \(\gamma\). For readability, we plot every 10th retained draw in Figure \ref{exd:fig:ex1mcmc}.

\begin{figure}[t]
\centering
\includegraphics[width=0.82\textwidth]{figures/ch02-exdqlm/ex1mcmc.png}
\caption{Example 1: Lake Huron. Trace and density plots from the median run with 7000 burn-in iterations and 3000 retained draws. For readability, every 10th retained draw is used in the plotting step. The top row shows the posterior trace plots for \(\sigma\) and \(\gamma\), and the bottom row shows the corresponding posterior densities. The displayed paths are those produced by the recorded reference run.}
\label{exd:fig:ex1mcmc}
\end{figure}
...
For our next example, we use the yearly Sunspot time series from the package \pkg{datasets}.
...
County, CA \citep{exd-btflow}. The \pkg{exdqlm} version 1.1.1 used for this
...
The essential data alignment is shown below. The replication script uses the
same date construction, merge, training-window split, and training-window
scaling shown in the code blocks for this example.



These data can be seen in Figure \ref{exd:fig:ex3data} and plotted as follows.
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.9\textwidth]{figures/ch02-exdqlm/ex3zetapsi.png}
...
Figure \ref{exd:fig:ex3tftheta} then displays the augmented transfer states
directly. As described in Section \ref{exd:sec:tf}, \(\zeta_t\) controls the
...
\(\psi_{\mathrm{AMO},t}\), respectively. The NOI coefficient is estimated below
zero, while the AMO coefficient is close to zero and its pointwise 95\% CrI
overlaps zero. We therefore use this analysis to illustrate the specification,
...
\begin{tabular}{@{}lrrr@{}}
 ...
 Model & KL & CRPS & PPLC \\
 \midrule
 \code{M0} no covariates & 0.122 & 0.416 & 177.944 \\
 \code{MREG} direct regression & \textbf{0.108} & 0.347 & 160.624 \\
 \code{MTF} transfer function & 0.149 & \textbf{0.314} & \textbf{141.694} \\
 \bottomrule
\end{tabular}
...
\begin{tabular}{@{}llrrrr@{}}
 ...
 \midrule
 \tworowcell{0.05} & LDVB & \textbf{50.49} & 0.108 & \textbf{0.034} & 0.673 \\
 & MCMC & 104.39 & \textbf{0.069} & 0.035 & \textbf{0.594} \\
 \midrule
 \tworowcell{0.25} & LDVB & \textbf{43.30} & 0.310 & 0.078 & 0.371 \\
 & MCMC & 110.19 & \textbf{0.271} & \textbf{0.065} & \textbf{0.329} \\
 \midrule
\tworowcell{0.50} & LDVB & \textbf{22.01} & \textbf{0.128} & 0.103 & 0.341 \\
 & MCMC & 142.12 & 0.141 & \textbf{0.083} & \textbf{0.335} \\
 \bottomrule
\end{tabular}}
...
The results reinforce the visual comparison in Figure \ref{exd:fig:ex4static}. The MCMC posterior-simulation fit has the smaller holdout quantile RMSE in all three fits, although the median difference is small. MCMC gives smaller active-signal RMSE at \(p_0 = 0.05\) and \(p_0 = 0.25\), while LDVB is slightly smaller at \(p_0 = 0.50\). Null-coefficient MAE is slightly smaller for LDVB at \(p_0 = 0.05\) and for MCMC at \(p_0 = 0.25\) and \(p_0 = 0.50\). The LDVB fit is nevertheless faster in all three cases, with runtime reductions of roughly 2.1-fold at \(p_0 = 0.05\), 2.5-fold at \(p_0 = 0.25\), and 6.5-fold at \(p_0 = 0.50\). In this benchmark, the LDVB approximation is useful as a rapid first-pass screen, while static MCMC provides a more expensive posterior-simulation comparison when interval accuracy or posterior tail behavior is central.
...
Several development directions follow from these limitations: multivariate-output dynamic quantile models that represent cross-series dependence through a modeled correlation matrix for related time series; further assessment of variational approximation error for the nonconjugate \((\gamma,\sigma)\) skewness--scale block; nonlinear dynamic quantile modeling beyond the linear state-space form used here; and joint or regularized fitting across quantile levels. The user-facing sections, integrated technical material, and replication sources have separate roles: the user-facing sections describe the workflow, the integrated technical material records selected posterior targets and package-specific computational blocks, and the submitted replication script regenerates the reported figures, tables, selected fitted-object output, and benchmark information.
